 ...
 * CAPA → PDF pipeline.
 *
 * Step 1 (buildCapaReport): serialise the *entire* D1–D7 answer set of a
 *   CAPACase into a structured JSON report.
...
export function renderReportHtml(report: CapaReport, logoSrc: string): string {
  const m = report.meta;
  ...
  return `
    <div style="width:794px;box-sizing:border-box;padding:30px 34px 40px;background:#ffffff;font-family:'Inter','Helvetica Neue',Arial,sans-serif;color:${INK};">
      ...
      <div class="report-section">${d6}</div>
      <div class="report-section">${d7}</div>
...
export async function downloadCapaPdf(capa: CAPACase): Promise<void> {
  const { default: html2pdf } = await import("html2pdf.js");

  const report = buildCapaReport(capa);
  const logoSrc = await toDataUrl(logoUrl);
  const html = renderReportHtml(report, logoSrc);
  ...
  // unit:"mm" + format:"a4" lets html2pdf scale the 794px canvas to fit the
  // page width (px units would place it 1:1 and clip the right edge).
  const pdfOptions = {
    margin: [10, 8, 10, 8],
    filename: `${capa.id}-CAPA-Report.pdf`,
